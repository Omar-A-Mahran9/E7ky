...
var KTDatatablesServerSide = (function () {
    ...
    //             let currentBtnIndex = $(statusButtons).index(d)
    //             let data = datatable.row(currentBtnIndex).data();
    //             loadingAlert(__("Updating..."))
    //             if (data.vendor_shipment === null) {
    //                 errorAlert(__('You must add shipping details first'))
    //             } else {
    //                 $.ajax({
    //                     type: "post",
    //                     url: `/dashboard/change/vendor/${data.id}/status`,
    //                     data: {
    //                         status: data.approved == 2 || data.approved == 0 ? 1 : 2
    //                     },
    //                     success: function () {
    //                         datatable.draw();

    //                         if (data.approved == 0 || data.approved == 2) {
    //                             successAlert(`${__('Vendor approved successfully')} `);
    //                         } else {
    //                             successAlert(`${__('Vendor rejected')} `);
    //                         }
    //                     },
    //                     error: function (err) {
