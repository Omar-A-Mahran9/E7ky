                    ...
                    render: function (data, type, row) {
                        return `
                            <div>
                                <!--begin::Info-->
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class="mb-1 text-gray-800 text-hover-primary">${row.type}</a>
                                </div>
                                <!--end::Info-->
                            </div>
                        `;
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach((d) => {
            ...
            d.addEventListener("click", function (e) {
                ...
                $(".cover-input .image-input-wrapper").css(
                    "background-image",
                    `url('${data.full_cover_picture}')`
                );
                $("#password_inp").val("");
                ...
                $("#status_inp").val(data.status).trigger("change");

                $("#phone_inp").val(data.phone);
                $("#facebook_link_inp").val(data.facebook_link);
                $("#instagram_link_inp").val(data.instagram_link);

                $("#X_link_inp").val(data.X_link);
                $("#crud_form").attr(
                    "action",
